...
(function (angular, navigator) {
    ...
    function getItemTemplate (document, treeElem) {
        ...
        while (treeElem.childNodes.length > 0) {
            ...
            if (childNode.nodeName === 'LI') {
                ...
                if (createWrapper) {
                    ...
                    // wrapperEl.setAttribute('tag', 'span');
                    // var wrapperE2 = document.createElement('I');
                    // wrapperE2.setAttribute('tag', 'i');
                    // wrapperE2.setAttribute('ng-class', "{1: 'icon-leaf' , 0: 'icon-minus-sign'}[item1.isLeaf]");
                    // wrapperEl.appendChild(wrapperE2);
                ...
                // var oldchild = childNode.children[1];
                // childNode.replaceChild(ulEl, oldchild);

                var ulEl = document.createElement('UL');
                ulEl.className = treeElem.className;
                // childNode.appendChild(ulEl);
                childNode.insertBefore(ulEl, childNode.children[1]);
        ...
        return itemTemplate || angular.element('<li class="ng-tree-node"><span tag="span">{{item}}</span><ul></ul></li>');
        // var liHtml = '<li ng-class="ng-tree-node">'
        //             +   '<span>'
        //             +       '<i tag="i" ng-class="icon-leaf"></i> {{item}}'
        //             +   '</span>'
        //             +   '<ul></ul>'
        //             +'</li>';
        // return itemTemplate || angular.element(liHtml);
    }
    ...
    app.directive('ngTree', ['$compile', '$document', function ($compile, $document) {
        return {
